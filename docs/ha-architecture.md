...
- **Records source of truth:** repo files in `pihole/dnsmasq.d/*.conf`
  ...
  `configs: content:`; pihole-1 LXC receives `06-host-overrides.conf`
  automatically via `scripts/sync-host-overrides.py --apply` (handles
  `pct push` + pihole-FTL reload). The two static zone files
  (`04-d-lcamaral-com.conf`, `05-home.conf`) are still hand-pushed via
  `pct push` when they change — see `pihole/README.md`.
